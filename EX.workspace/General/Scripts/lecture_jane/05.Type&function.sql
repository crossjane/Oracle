...
--숫자를 문자열로 변환
SELECT TO_CHAR(10000000, '999,999,999') -- 9자리 숫자까지 숫자를 표기하고 3자리마다 쉼표 표시 
...
--TO_DATE(문자열 데이터, 문자열 데이터의 날짜 형식): 매개변수로 지정된 날짜 형식으로 되어있는 문자열을 DATE타입으로 변환 후 리턴 
SELECT TO_DATE('20240524154900', 'YYYYMMDDHH24:MI:SS')
 	, TO_DATE('20240524 15', 'YYYYMMDD HH24')
 	, TO_DATE('24', 'YY')
 	, TO_DATE('24', 'RR')
 	, TO_DATE('99', 'YY')
 	, TO_DATE('99', 'RR')
...
--TO_NUMBER(문자열 데이터, 문자열 데이터의 숫자 형식)	: 매개변수로 지정된 숫자 형식으로 된 문자열을 NUMBER 타입으로변환 후 리턴 
...
--ROUND(실수), TRUNC(실수) : 정수까지(소수점 첫째자리에서) 반올림, 정수까지(소수점 첫째자리에서) 버림
 SELECT  ROUND(1.45, 0)
 	 , ROUND(1.45)
 	 FROM DUAL;
...
--담당교수가 NULL 인 교수정보는 교수 배정되지 않음이라고 조회하고 
--과목이 NULL 교수의 과목 정보는 과목배정받지 못함이라고 조회	
SELECT NVL(C.CNO , '과목배정받지 못함')
	, NVL(C.CNAME , '과목배정받지 못함')
...
--			컬럼의 데이터나 조회해온 데이터가 조건 값1과 일치할때 실행될 내용,
...
--			컬럼의 데이터나 조회해온 데이터가 조건 값1과 일치할때 실행될 내용,
--			조건 값2,
--			컬럼의 데이터나 조회해온 데이터가 조건 값2과 일치할때 실행될 내용,
--			조건 값3 
--			컬럼의 데이터나 조회해온 데이터가 조건 값3과 일치할때 실행될 내용,
--			...
--			조건값 N,
--			컬럼의 데이터나 조회해온 데이터가 조건 값N과 일치할때 실행될 내용,
--			컬럼의 데이터나 조회해온 데이터가 조건 값N과 일치하지 않을때 실행될 내용(위의 조건과 모두 일치하지 않는경우))
--DECODE는 조건문을 처리하는 구문으로 조건에는 값만 넣어서 사용할 수 있다.
...
--업무가 개발이면 50% 인상, 업무가 경영이면 30% 인상, 지원이면 20%인상 , 그 외 업무면 10% 인상된 인상급여 조회

SELECT ENO
	, ENAME 
	, JOB
	, SAL
--	, NVL(COMM,'보너스없음')  NULL 의 형식이 문자가 아니라 적용이 안됨.
	, DECODE(JOB,
		 '개발', '50%인상',
		 '경영','30%인상',
		 '지원','20%인상',
