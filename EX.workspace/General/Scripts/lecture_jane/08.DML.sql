...
--데이터를 저장, 수정, 삭제 하는 언어로 INSERT INTO, UPDATE SET, DELETE FROM이 존재한다. 
--DML은 트랜잭션을 완료하기 위해서 항상 COMMIT 이나 ROLLBACK을 동반한다.
...
--INSERT INTO 시 지정한 컬럼의 개수와 컬럼의 타입에 맞는 데이터를 입력해야한다.
INSERT INTO EMP(ENO, ENAME, JOB, HDATE) VALUES ('9993', '정도전', '경영');
INSERT INTO EMP(SAL) VALUES ('AAAAA');
--VARCHAR타입의 숫자 값이 NUBER타입으로 형변환이 일어나면서 저장된다.
INSERT INTO EMP(SAL) VALUES ('12345');

--모든 컬럼의 데이터를 지정할 때는 컬럼 지정을 생략해도 된다.
INSERT INTO EMP (ENO , ENAME , JOB , MGR , HDATE, SAL , COMM, DNO)
	VALUES ('9996','임꺽정','회계','0001',SYSDATE, 4000, 3000, '10');

INSERT INTO EMP VALUES ('9995','조병조','경영','0201',SYSDATE, 3700, 280, '20');
COMMIT;

--다량의 데이터를 SELECT구문을 이용해서 저장 
...
--DNO 부서번호가 30인 사원 목록을 조회해서 EMP_DNO30에 저장
...
--담당교수가 없는 과목은 담당교수번호화 이름은 NULL로 저장 
...
--WHERE절을 사용하지 않으면 모든 데이터가 수정된다. **항상 WHERE절 써야함.
UPDATE EMP_DNO30 
	 SET 
	 	MGR = '0002';
	 COMMIT;
...
--WHERE절로 특정 데이터만 수정 
--EMP_DNO30 테이블에서 ENO 2007인 사원의 급여를 3100DMFH TNWJD 
UPDATE EMP_DNO30 
 	 SET 
 	 	SAL = 3100
 	 	WHERE ENO = 2007;
...
--여러개의 컬럼 데이터를 한번에 수정할때는 수정할 컬럼을 , 로 묶어서 지정할 수 있다. 	
--EMP_DNO30테이블에서 ENO이 3004인 사원의 업무를 경영으로, MGR은 2003DMFH HDATE는 오늘 날짜로 SAL은 5000으로 
--COMM는 500으로 수정 
UPDATE EMP_DNO30
	SET 
	 JOB = '경영',
...
--EMP_DNO30테이블에서 DNO이 20, 30 부서를 60 부서로 통합하고 
...
--서브쿼리를 이용한 데이터 수정 
--SET절에 서브쿼리의 결과로 나오는 컬럼의 개수만큼  ()묶은 컬럼을 지정하고 = 다음에 서브쿼리를 작성한다. 
--()로 묶인 컬럼의 개수와 타입이 서브쿼리의 결과의 데이터의 컬럼개수와 타입이 일치해야한다. 
--서브쿼리의 결과는 한 행만 조회가 되어야 한다.

CREATE TABLE DEPT_COPY1
	 AS SELECT * FROM DEPT;

--DEPT_COPY1 테이블에서 DNO 20, 30 인 부서를 50 부서와 통합 
UPDATE DEPT_COPY1
	SET 
...
--WHERE절을 사용하지 않으면 테이블의 모든 데이터를 삭제한다. 
DELETE FROM DEPT_COPTY1;
COMMIT;
...
DELETE FROM STUDENT_COPY1
		WHERE SNO IN (
		--AVG(RESULT)값은 여기에 들어갈 필요가 없다. WHERE SNO 와 맞춰주면된다.
					SELECT SNO 	
					FROM SCORE 
					GROUP BY SNO
