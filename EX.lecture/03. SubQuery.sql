...
--2. 집합연산자 
--집한연산자는 서로 다른 두 쿼리의 결과를 합집합, 차집합, 교집합 해주는 연산자 
--2-1. 합집합 연산자(UNION, UNION ALL)
--2000년 이후에 부임된 교수의 교수번호, 교수이름, 부임일자와 2000년 이후에 채용된 사원의 사원번호 , 사원이름, 채용일자를 조회 
--첫번째 쿼리에서 컬럼의 개수, 데이터 타입이 결정되기 떄문에 두번째 쿼리는 첫번째 쿼리의 컬럼의 갯수 , 데이터 타입을 따라야 한다. 
SELECT PNO 
	 , PNAME 
	 , HIREDATE
	 FROM PROFESSOR 
...
--평점이 3.0이상인 학생의 학생번호, 학생이름, 학년, 평점과 학년이 3학년인 학생의 학생 번호, 학생이름, 학년, 평점을 합께 조회
--중복제거.
...
--2-2.차집합 연산자 (MINUS)
--첫번째 쿼리에서 두번쨰 쿼리와 공통된 데이터를 제외한 결과를 조회한다. 

INSERT INTO EMP VALUES('9999','제갈궁', '지원', NULL, SYSDATE, 3200, 320, NULL);
COMMIT;
...
SELECT CNO 
	 ...
	 FROM COURSE 
	 WHERE 1=1 
	 -- WHERE절이 아래에 있는데 위에 없으면 빨간색 글씨가 떠서 항상 참인 구문으로 같이 맞춰주면 된다. 
MINUS
SELECT CNO 
	 , CNAME
	 , PNO
...
--교집합 연산자를 사용해서 물리, 화학과 학생중 평점이 3.0 이상인 학생의 학생번호 
