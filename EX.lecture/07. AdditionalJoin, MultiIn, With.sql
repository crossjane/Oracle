...
--서브쿼리를 너무 많이 사용하면 쿼리의 속도가 현저하게 느려지기 때문에 WITH절을 적절히 잘 사용해야한다.
--SELECT 구문 위에 WITH 절로 가상테이블을 구성한다. 
--WITH절은 복잡한 SQL에서 동일 블록에 대해 반복적으로 SQL문을 사용하는 경우 그 블록에 이름을 부여하여 재사용 할 수 있게 함으로서 쿼리 성능을 높일 수 있음.
WITH DNO10 AS(
	SELECT ENO
		 , ENAME 
		 , DNO 
...
--기말고사 성적의 평균이 50점 이상인 과목번호, 과목이름, 기말고사 성적의 평균점수를 가지는 가상테이블 
--가상테이블 OVER50 를 WITH 절로 구현하고 해당 과목을 수강하는 학생들의 학생 정보조회
...
--WITH 절로 두개 이상의 가상테이블을 만들 떄는 ,로 연결해서 만든다.
...
--과목명에 화학이 포함된 과목의 과목번호, 과목이름, 기말고사 성적, 학생번호를 가지는 가상테이블 CHMRES WITH 절로 구현하고 
--화학과 1학년 학생의 학생번호, 학생이름, 학생의 기말고사 성적의 평균점수(소수점 둘째 자리까지표시)를 조회
