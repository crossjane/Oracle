...
--그룹함수를 사용할 때 주의할 점은 SELECT절에 포함된 모든 컬럼들은 GROUP BY 절에 명시돼야 한다. 
--학년별 학생들의 평점의 평균을 구하는 그룹함수 
--GROUP BY 기준 값으로 같은 것을 하나의 데이터 행으로 처리해버림. 같은값이 없을땐 걍 낱개처리됨.
...
-- SELECT 절에 포함된 컬럼들이 모두 GROUP BY에 명시돼야 하는 규칙때문에 데이터가 변질 된다.
SELECT SYEAR
	, MAJOR 
	, AVG(AVR)
	FROM STUDENT
...
--과목별 기말고사 최고점 조회하는데 과목번호, 과목이름, 학생이름,  점수 조회
SELECT A.CNO
	 , A.CNAME 
	 , ST.SNO
	 , ST.SNAME
